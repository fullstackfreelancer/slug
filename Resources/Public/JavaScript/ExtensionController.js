document.addEventListener("DOMContentLoaded", function() {
    ...
    let slugField = 'slug'
    let recordTable = 'tx_showcase_domain_model_project'
    ...
    function buildHTML(records){
        ...
        for (var i = 0; i < records.length; i++) {
            ...
            output += '</div>';
            output += '<div class="col py-1">';
                output += '<div class="input-group"><span class="input-group-addon">'+records[i]['sys_language_uid']+'</span><input type="text" class="form-control slug-input" value="'+slug+'"/></div>';
            output += '</div>';
        ...
        rows.forEach((row, i) => {
            ...
            if(button_generate){
                button_generate.addEventListener('click',function(e){
                    slugHelper.generate(uid, sitePrefix, recordTable, titleField, slugField);
                });
            }
